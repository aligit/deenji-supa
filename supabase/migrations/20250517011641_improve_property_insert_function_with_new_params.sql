-- Migration: 20250517140000_add_area_and_year_built_parameters.sql
...
-- Create the updated function with additional parameters for area and year_built
CREATE OR REPLACE FUNCTION insert_property_direct(
  ...
  p_area NUMERIC DEFAULT NULL,            -- Add area parameter
  p_year_built INTEGER DEFAULT NULL       -- Add year_built parameter
)
RETURNS BIGINT
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  idx INTEGER;
  attr JSONB;
  area_value NUMERIC := p_area;          -- Initialize with passed parameter
  year_built_value INTEGER := p_year_built; -- Initialize with passed parameter
  bedrooms_value INTEGER := p_bedrooms;   
...
BEGIN
  ...
  -- Extract specific attribute values from the attributes JSONB array only if direct values weren't provided
  IF p_attributes IS NOT NULL AND jsonb_array_length(p_attributes) > 0 THEN
    FOR attr IN SELECT * FROM jsonb_array_elements(p_attributes) LOOP
      ...
      CASE attr->>'title'
        WHEN 'متراژ' THEN
          -- Only extract area if not already provided
          ...
          END IF;
        WHEN 'ساخت' THEN
          -- Only extract year_built if not already provided
          IF year_built_value IS NULL THEN
            cleaned_value := regexp_replace(attr->>'value', '[^\d]', '', 'g');
            IF cleaned_value IS NOT NULL AND cleaned_value != '' THEN
...
Updated with direct parameters for area and year_built.
Prioritizes direct parameters over attribute extraction for all fields.';
