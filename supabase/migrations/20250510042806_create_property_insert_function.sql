-- supabase/migrations/<timestamp>_fix_numeric_handling_in_property_insert.sql

-- Update the function to handle empty strings and null values properly
...
DECLARE
  ...
  floor_material_value TEXT;
  bathroom_type_value TEXT;
  cooling_system_value TEXT;
  heating_system_value TEXT;
  hot_water_system_value TEXT;
  cleaned_value TEXT;
BEGIN
  ...
  -- Extract specific attribute values from the attributes JSONB array
  IF p_attributes IS NOT NULL AND jsonb_array_length(p_attributes) > 0 THEN
    FOR attr IN SELECT * FROM jsonb_array_elements(p_attributes) LOOP
      CASE attr->>'title'
        ...
        ELSE
          -- Check for complex titles (like floor material, bathroom type, etc.)
          IF attr->>'title' LIKE '%جنس کف%' THEN
            floor_material_value := TRIM(REPLACE(attr->>'title', 'جنس کف ', ''));
          ELSIF attr->>'title' LIKE '%سرویس بهداشتی%' THEN
            bathroom_type_value := TRIM(REPLACE(attr->>'title', 'سرویس بهداشتی ', ''));
          ELSIF attr->>'title' LIKE '%سرمایش%' THEN
            cooling_system_value := TRIM(REPLACE(attr->>'title', 'سرمایش ', ''));
          ELSIF attr->>'title' LIKE '%گرمایش%' THEN
            heating_system_value := TRIM(REPLACE(attr->>'title', 'گرمایش ', ''));
          ELSIF attr->>'title' LIKE '%تأمین‌کننده آب گرم%' THEN
...
Updated to properly handle empty strings and null values when converting to numeric types.';
